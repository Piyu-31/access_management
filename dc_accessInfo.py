...
def setDCData():
  ...
  dcInfo["org_id"]=org_id.strip()
  dcInfo["email"]=email.strip()
  dcInfo["first_name"]=first_name.strip()
  dcInfo["last_name"]=last_name.strip()
  dcInfo["user_type"]=user_type.strip()
  dcInfo["user_contact"]=user_contact.strip()
  if not session['edit_record']:
    Password_Un = generate_password(4,2)
    dcInfo["password"]=Password_Un.strip()
  dcInfo["created_date"] = date_time.strip()
